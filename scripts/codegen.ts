#!/usr/bin/env npx tsx
/**
 * Build Anchor IDLs and generate TypeScript codecs via Codama.
 *
 * Runs `anchor idl build` from the repo root for each program, writing IDL
 * JSON to target/idl/, then generates TypeScript into src/generated/.
 *
 * Usage: npm run generate:codecs
 */
...
const GENERATED_DIR = path.join(SDK_ROOT, 'src', 'solana', 'generated');
...
console.log('Building Anchor IDLs...');
for (const program of PROGRAMS) {
  const idl_out_path = path.join(IDL_DIR, `${program.idl_name}.json`);
  execSync(
    `anchor idl build --skip-lint -p ${program.idl_name} --out ${idl_out_path}`,
    { cwd: REPO_ROOT, stdio: 'inherit' },
  );
  console.log(`  built ${program.idl_name}`);
}
