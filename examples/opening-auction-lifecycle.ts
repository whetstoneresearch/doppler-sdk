...
async function main() {
  ...
  console.log(
    '  Min proceeds:',
    formatEther(params.doppler.minProceeds),
    'WETH',
  );
  console.log(
    '  Max proceeds:',
    formatEther(params.doppler.maxProceeds),
    'WETH',
  );

  console.log('\nStep 1: simulate create');
  const createSimulation =
    await sdk.factory.simulateCreateOpeningAuction(params);
  console.log('  Predicted token:', createSimulation.tokenAddress);
  console.log(
    '  Predicted opening hook:',
    createSimulation.openingAuctionHookAddress,
  );
  ...
  const openingAuction = await sdk.getOpeningAuction(
    stateBefore.openingAuctionHook,
  );
  const phaseBefore = await openingAuction.getPhase();
  console.log(
    '  Phase before settle:',
    phaseBefore,
    `(${phaseLabel(phaseBefore)})`,
  );
  ...
  if (canRecover) {
    ...
    console.log(
      '  recoverOpeningAuctionIncentives skipped (simulation failed)',
    );
  }
