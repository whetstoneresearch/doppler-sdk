...
- Public devnet RPC for inspection, or a localhost validator for transactions. Devnet transaction submission is deliberately disabled until its deployment is verified against the refactor. The app rejects other endpoints; use the CLI examples for custom RPC setups.

## Walkthrough

1. Select **Local validator** for the transaction walkthrough and connect the wallet (or choose **Use local test wallet**). Create an oracle with 2–8 unique outcome labels, or paste an existing oracle. Labels encode UTF-8 bytes padded to 32 bytes; they must match the IDs used by other clients.
