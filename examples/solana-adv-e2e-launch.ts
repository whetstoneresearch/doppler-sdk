...
/**
 * Poll a launch account until its phase changes from TRADING or a timeout
 * elapses.  Returns the final launch state.
 */
async function pollUntilMigrated(
  rpc: Parameters<typeof initializer.fetchLaunch>[0],
  launchAddress: Address,
  intervalMs = 5_000,
  maxAttempts = 60,
): Promise<ReturnType<typeof initializer.fetchLaunch>> {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const launch = await initializer.fetchLaunch(rpc, launchAddress, { commitment: 'confirmed' })
    if (!launch) throw new Error(`Launch account not found: ${launchAddress}`)
    if (launch.phase !== initializer.PHASE_TRADING) return launch
    console.log(
      `  [${attempt + 1}/${maxAttempts}] Phase: TRADING — quoteDeposited: ${launch.quoteDeposited} lamports — waiting...`,
    )
    await new Promise(resolve => setTimeout(resolve, intervalMs))
  }
  return initializer.fetchLaunch(rpc, launchAddress, { commitment: 'confirmed' })
}
...
async function main() {
  ...
  ).then(r => r.json()).then((d: { solana: { usd: number } }) => d.solana.usd)
  ...
  try {
    ...
    const ownedLaunches = await initializer.fetchLaunchesByAuthority(rpc, payer.address)
    console.log(`  Found ${ownedLaunches.length} launch(es)`)
    for (const { address: addr, account } of ownedLaunches) {
      const phaseLabel =
        account.phase === initializer.PHASE_TRADING  ? 'TRADING'  :
        account.phase === initializer.PHASE_MIGRATED ? 'MIGRATED' :
        account.phase === initializer.PHASE_ABORTED  ? 'ABORTED'  :
        String(account.phase)
      console.log(`  ${addr}  phase=${phaseLabel}  quoteDeposited=${account.quoteDeposited}`)
    }
    console.log('')

    // ── Step 3: Preview a curve buy without executing ────────────────────────
    console.log('Step 3: Previewing bonding curve buy (read-only simulation)...')
    const BUY_AMOUNT_IN = 10_000_000n // 0.01 SOL in lamports
    ...
    const previewTxMsg = pipe(
      createTransactionMessage({ version: 0 }),
      msg => setTransactionMessageFeePayerSigner(payer, msg),
      msg => setTransactionMessageLifetimeUsingBlockhash(previewBlockhash, msg),
      msg => appendTransactionMessageInstructions([previewIx], msg),
    )
    const signedPreviewTx = await signTransactionMessageWithSigners(previewTxMsg)
    const { value: simulateResult } = await rpc.simulateTransaction(
      getBase64EncodedWireTransaction(signedPreviewTx),
    ...
    if (simulateResult.returnData?.data) {
      ...
      console.log('  Amount in:  ', BUY_AMOUNT_IN.toString(), 'lamports (0.01 SOL)')
      console.log('  Amount out: ', preview.amountOut.toString(), 'base atoms (estimated)')
      console.log('  Fee paid:   ', preview.feePaid.toString(), 'lamports')
    } else {
      console.log('  Preview simulation returned no data (launch may need trading to be open)')
    }
    console.log('')
    ...
    const createBaseAtaIx = {
      programAddress: ASSOCIATED_TOKEN_PROGRAM_ID,
      accounts: [
        { address: payer.address,          role: ACCOUNT_ROLE_WRITABLE_SIGNER },
        { address: userBaseAta,            role: ACCOUNT_ROLE_WRITABLE },
        { address: payer.address,          role: ACCOUNT_ROLE_READONLY },
        { address: baseMint.address,       role: ACCOUNT_ROLE_READONLY },
        { address: SYSTEM_PROGRAM_ID,      role: ACCOUNT_ROLE_READONLY },
        { address: TOKEN_PROGRAM_ID,       role: ACCOUNT_ROLE_READONLY },
      ],
    ...
    const createQuoteAtaIx = {
      ...
      accounts: [
        { address: payer.address,          role: ACCOUNT_ROLE_WRITABLE_SIGNER },
        { address: userQuoteAta,           role: ACCOUNT_ROLE_WRITABLE },
        { address: payer.address,          role: ACCOUNT_ROLE_READONLY },
        { address: WSOL_MINT,              role: ACCOUNT_ROLE_READONLY },
        { address: SYSTEM_PROGRAM_ID,      role: ACCOUNT_ROLE_READONLY },
        { address: TOKEN_PROGRAM_ID,       role: ACCOUNT_ROLE_READONLY },
      ],
      data: new Uint8Array([1]), // CreateIdempotent
    ...
    new DataView(transferData.buffer).setBigUint64(4, BUY_AMOUNT_IN + RENT_WSOL_ATA, true)
    const transferSolIx = {
...

    {
      ...
      const setupTxMessage = pipe(
        createTransactionMessage({ version: 0 }),
        msg => setTransactionMessageFeePayerSigner(payer, msg),
        msg => setTransactionMessageLifetimeUsingBlockhash(setupBlockhash, msg),
        msg => appendTransactionMessageInstructions(
          [createBaseAtaIx, createQuoteAtaIx, transferSolIx, syncNativeIx], msg,
        ),
      )
      const signedSetupTx = await signTransactionMessageWithSigners(setupTxMessage)
    ...
    console.log('  Curve buy confirmed:', getSignatureFromTransaction(signedSwapTx))
    console.log('')

    // ── Step 5: Poll until graduation ────────────────────────────────────────
    //
    // In a real integration, the graduation trigger is called by anyone once
    // quoteDeposited >= minRaiseQuote.  Here we simply poll the phase.
    // On devnet with only 1 SOL deposited (vs 50 SOL threshold), the launch
    // stays in TRADING — the polling loop will time out but demonstrates the
    // pattern for production use.
    console.log('Step 5: Polling launch phase until graduation...')
    console.log('  (On devnet with a 50 SOL threshold this will time out — expected)')

    const finalLaunch = await pollUntilMigrated(rpc, launch, 5_000, 3)

    if (!finalLaunch) {
      throw new Error('Launch account disappeared during polling')
    }

    const finalPhase =
      finalLaunch.phase === initializer.PHASE_TRADING  ? 'TRADING'  :
      finalLaunch.phase === initializer.PHASE_MIGRATED ? 'MIGRATED' :
      finalLaunch.phase === initializer.PHASE_ABORTED  ? 'ABORTED'  :
    ...
    if (finalLaunch.phase === initializer.PHASE_MIGRATED) {
      ...
      const [migratorStateAddress] = await cpmmMigrator.getCpmmMigratorStateAddress(launch)
      const migratorState = await cpmmMigrator.fetchCpmmMigratorState(rpc, migratorStateAddress)
      ...
      const poolResult = await getPoolByMints(rpc, baseMint.address, WSOL_MINT)

      if (poolResult) {
        const { address: poolAddress, account: pool } = poolResult
        const price0 = getSpotPrice0(pool) // base per WSOL
        const price1 = getSpotPrice1(pool) // WSOL per base
      ...
      console.log('Launch still in TRADING phase — skipping post-graduation steps.')
      console.log('Run createMigrateLaunchInstruction once quoteDeposited >= minRaiseQuote.')
    }
