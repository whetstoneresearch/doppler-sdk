...
if (!ASSET_ADDRESS)
  throw new Error(
    'ASSET_ADDRESS is not set. Run multicurve-lockable-beneficiaries.ts first.',
  );
...
async function main() {
  ...
  try {
    ...
    const isToken0Asset =
      tokenAddress.toLowerCase() < numeraireAddress.toLowerCase();
    ...
    console.log(
      '   - Call collectFees() again after more trading activity to collect new fees',
    );
    console.log(
      '   - Pool must have a non-zero fee tier for fees to accumulate',
    );
    console.log(
