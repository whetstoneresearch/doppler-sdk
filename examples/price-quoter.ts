...
async function main() {
  ...
  try {
    ...
    console.log(
      '- Price impact (ticks crossed):',
      v3Quote.initializedTicksCrossed,
    );
  ...
  try {
    ...
    console.log(
      '- Amount in required:',
      formatEther(v3QuoteOut.amountIn),
      'ETH',
    );
    console.log(
      '- Price impact (ticks crossed):',
      v3QuoteOut.initializedTicksCrossed,
    );
  ...
  results.forEach((result, i) => {
    console.log(
      `${i + 1}. ${result.version}: ${formatEther(result.amountOut)} USDC (gas: ${result.gas})`,
    );
  });
