/**
 * Example: Deploy Decay Multicurve on Base Mainnet + Simulate Swaps
 *
 ...
 *
 * Optional env:
 * - ALCHEMY_API_KEY (preferred; builds Base mainnet Alchemy URL)
 * - ALCHEMY_BASE_MAINNET_RPC_URL or ALCHEMY_RPC_URL (explicit Alchemy RPC URL)
...
const DECAY_DURATION_SECONDS = 15;
const SAMPLE_AMOUNT_IN = parseEther('0.0001');
const SIMULATION_SLIPPAGE_BPS = 50n; // 0.50%
const NUMERAIRE_PRICE_USD = 3_000;
...
async function simulateUniversalRouterSwap(params: {
  publicClient: {
  ...
  await params.publicClient.simulateContract({
    address: params.universalRouterAddress,
    abi: universalRouterAbi,
    functionName: 'execute',
    args: [params.commands, params.inputs, params.deadline],
...
async function main() {
  ...
  if (!addresses.v4DecayMulticurveInitializer) {
    throw new Error(
      'Decay multicurve initializer is not configured for Base mainnet'
    );
  }

  console.log('Base Mainnet Decay Multicurve Swap Simulation Example');
  ...
  const params = sdk
    ...
    .tokenConfig({
      name: 'Decay Test',
      symbol: 'DTEST',
      tokenURI: 'ipfs://decay-test.json',
    })
    .saleConfig({
      initialSupply: 100_000_000_000n * WAD,
      numTokensToSell: 100_000_000_000n * WAD,
      numeraire: addresses.weth,
    })
    .withCurves({
      numerairePrice: NUMERAIRE_PRICE_USD,
  ...
  console.log(
    '  Deployment is onchain; all swap checks below are simulation-only (eth_call).'
  );
  console.log();
  ...
  for (const checkpoint of checkpoints) {
    ...
    await simulateUniversalRouterSwap({
      publicClient,
      universalRouterAddress: addresses.universalRouter,
      commands,
      inputs,
  ...
  await simulateUniversalRouterSwap({
    publicClient,
    universalRouterAddress: addresses.universalRouter,
    commands: terminalCommandsOne,
    inputs: terminalInputsOne,
    deadline: terminalDeadlineOne,
    account: account.address,
    value: SAMPLE_AMOUNT_IN,
  });
  ...
  await simulateUniversalRouterSwap({
    publicClient,
    universalRouterAddress: addresses.universalRouter,
    commands: terminalCommandsTwo,
    inputs: terminalInputsTwo,
    deadline: terminalDeadlineTwo,
    account: account.address,
    value: SAMPLE_AMOUNT_IN,
  });
