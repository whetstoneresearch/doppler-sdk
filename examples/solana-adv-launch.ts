...
async function main() {
  ...
  const BASE_FOR_CURVE = BASE_TOTAL_SUPPLY - BASE_FOR_DISTRIBUTION - BASE_FOR_LIQUIDITY // 75%
  ...
  console.log('  Distribution:   ', BASE_FOR_DISTRIBUTION.toString(), 'atoms (20%)')
  console.log('    Creator:      ', CREATOR_SHARE.toString())
  console.log('    Team:         ', TEAM_SHARE.toString())
  console.log('  CPMM liquidity: ', BASE_FOR_LIQUIDITY.toString(), 'atoms (5%)')
  console.log('')
  ...
  try {
    ...
    if (launchAccount) {
      ...
      console.log('  Phase:              ', launchAccount.phase === initializer.PHASE_TRADING ? 'TRADING' : String(launchAccount.phase))
      console.log('  Curve fee:          ', launchAccount.curveFeeBps, 'bps')
      console.log('  Base for curve:     ', launchAccount.baseForCurve.toString())
      console.log('  Base for liquidity: ', launchAccount.baseForLiquidity.toString())
      console.log('  Base for distrib:   ', launchAccount.baseForDistribution.toString())
