...
if (!process.env.MINT_0 || !process.env.MINT_1) {
  throw new Error('MINT_0 and MINT_1 must be set to the two token mints of the pool')
}
...
const ASSOCIATED_TOKEN_PROGRAM: Address = address('ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJe8bRo')
...
async function main() {
  ...
  const poolResult = await getPoolByMints(rpc, MINT_0, MINT_1)
  ...
  const quote = getSwapQuote(pool, AMOUNT_IN, direction)
  ...
  console.log('  Amount out:    ', quote.amountOut.toString(), '(token1 atoms, estimated)')
