...
async function main() {
  ...
  console.log(
    'Airlock owner beneficiary:',
    airlockBeneficiary.beneficiary,
    `(${airlockBeneficiary.shares} shares)`,
  );
  ...
  console.log(
    '  vesting: 100M tokens over 1 year (70M deployer + 30M advisor)',
  );
  ...
  for (let attempt = 1; attempt <= maxPreflightAttempts; attempt++) {
    ...
    console.log(
      `Mined vanity salt (attempt ${attempt}/${maxPreflightAttempts}):`,
    );
    ...
    try {
      ...
      if (cause) {
        ...
        if (cause.shortMessage)
          console.log('  cause:', String(cause.shortMessage).split('\n')[0]);
        // Sometimes there's a nested cause (e.g. RPC error)
        const innerCause = cause.cause as Record<string, unknown> | undefined;
        if (innerCause) {
          if (innerCause.reason)
            console.log('  inner reason:', innerCause.reason);
          if (innerCause.data) console.log('  inner data:', innerCause.data);
          if (innerCause.shortMessage)
            console.log(
              '  inner cause:',
              String(innerCause.shortMessage).split('\n')[0],
            );
          if (innerCause.message)
            console.log(
              '  inner message:',
              String(innerCause.message).split('\n')[0],
            );
        }
      }
      ...
      if (err.details)
        console.log('  details:', String(err.details).split('\n')[0]);
