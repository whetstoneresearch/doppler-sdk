...
import { marketCapRangeToTicksForCurve, validateMarketCapParameters } from "../utils";
import {
  isNoOpEnabledChain,
...
export class MulticurveBuilder<C extends SupportedChainId> implements BaseAuctionBuilder<C> {
  ...
  poolConfig(params: {
    fee: number;
    tickSpacing: number;
    curves: { tickLower: number; tickUpper: number; numPositions: number; shares: bigint }[];
  ...
  withCurves(params: MulticurveMarketCapCurvesConfig): this {
    ...
    const tickSpacing = params.tickSpacing ?? (TICK_SPACINGS as Record<number, number>)[fee];
