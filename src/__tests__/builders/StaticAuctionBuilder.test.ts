  ...
  describe('withBeneficiaries', () => {
    it('sorts beneficiaries by address during build', () => {
      const beneficiaries = [
        ...
        { beneficiary: '0x0000000000000000000000000000000000000002' as Address, shares: (WAD * 3n) / 4n },
      ]
    ...
    it('includes beneficiaries in pool config when provided', () => {
      const beneficiaries = [
        { beneficiary: '0x0000000000000000000000000000000000000001' as Address, shares: WAD / 10n },
        { beneficiary: '0x0000000000000000000000000000000000000002' as Address, shares: (WAD * 9n) / 10n },
      ]

      const builder = StaticAuctionBuilder.forChain(CHAIN_IDS.BASE)
        .tokenConfig({ type: 'standard', name: 'Test', symbol: 'TST', tokenURI: 'ipfs://test' })
        .saleConfig({ initialSupply: 1_000n * WAD, numTokensToSell: 500n * WAD, numeraire: ZERO_ADDRESS })
    ...
    it('handles case-insensitive address sorting', () => {
      const beneficiaries = [
        { beneficiary: '0xABCDEF0000000000000000000000000000000001' as Address, shares: WAD / 2n },
        { beneficiary: '0xabcdef0000000000000000000000000000000000' as Address, shares: WAD / 2n },
      ]
      ...
      expect(builtBeneficiaries[0].beneficiary.toLowerCase()).toBe('0xabcdef0000000000000000000000000000000000')
      expect(builtBeneficiaries[1].beneficiary.toLowerCase()).toBe('0xabcdef0000000000000000000000000000000001')
    })
    ...
    it('preserves pool config when beneficiaries are added', () => {
      const builder = StaticAuctionBuilder.forChain(CHAIN_IDS.BASE)
        ...
        .withBeneficiaries([
          { beneficiary: '0x0000000000000000000000000000000000000001' as Address, shares: WAD },
        ])
        .withGovernance({ type: 'default' })
        .withMigration({ type: 'noOp' })
        .withUserAddress('0x00000000000000000000000000000000000000AA' as Address)
