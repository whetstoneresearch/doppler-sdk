...
describe("MulticurveBuilder", () => {
  ...
  it("configures curves from market cap presets using defaults", () => {
    ...
    expect(filler.tickUpper).toBe(
      filler.tickLower + DEFAULT_MULTICURVE_NUM_POSITIONS[2] * expectedTickSpacing,
    );
  });
  ...
  describe("withCurves", () => {
    ...
    it("throws if numPositions is not positive", () => {
      ...
      expect(() => {
        builder.withCurves({
          numerairePrice: 3000,
          curves: [{ marketCap: { start: 500_000, end: 1_000_000 }, numPositions: 0, shares: WAD }],
        });
    ...
    it("throws if market cap is not positive", () => {
      ...
      expect(() => {
        builder.withCurves({
          numerairePrice: 3000,
          curves: [{ marketCap: { start: 0, end: 1_000_000 }, numPositions: 10, shares: WAD }],
        });
