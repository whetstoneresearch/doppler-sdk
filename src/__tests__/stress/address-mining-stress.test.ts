...
describe('Address Mining Stress Test - Base Chain + WETH', () => {
  ...
  describe('Stress test', () => {
    it(
      `should run ${STRESS_CONFIG.iterations} mining iterations and track failures`,
      async () => {
        ...
        console.log(`Per-iteration timeout: ${STRESS_CONFIG.iterationTimeoutMs}ms`)
        ...
        for (let i = 0; i < STRESS_CONFIG.iterations; i++) {
          ...
          try {
            ...
            result.saltIterations = parseSaltIterations(encoded.createParams.salt)
            ...
            summary.minSaltIterations = Math.min(summary.minSaltIterations, result.saltIterations)
            summary.maxSaltIterations = Math.max(summary.maxSaltIterations, result.saltIterations)
        ...
        if (saltIterationsList.length > 0) {
          summary.avgSaltIterations = saltIterationsList.reduce((a, b) => a + b, 0) / saltIterationsList.length
        }
        ...
        console.log(`Iterations Completed: ${results.length}/${STRESS_CONFIG.iterations}`)
        ...
        console.log(`  Average salts tried: ${summary.avgSaltIterations.toFixed(0)}`)
      ...
      600_000
    ) // 10 minute timeout for stress test
  })
