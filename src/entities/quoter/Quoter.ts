...
export class Quoter {
   ...
   */
  async quoteExactInputV2(params: { amountIn: bigint; path: Address[] }): Promise<bigint[]> {
    const addresses = getAddresses(this.chainId);
   ...
   * @returns Array of amounts for each step in the path
   */
  async quoteExactOutputV2(params: { amountOut: bigint; path: Address[] }): Promise<bigint[]> {
    const addresses = getAddresses(this.chainId);
