...
export class DopplerFactory<C extends SupportedChainId = SupportedChainId> {
      ...
      poolInitializer: (() => {
        if (hasBeneficiaries) {
          const lockableInitializer =
            params.modules?.lockableV3Initializer ?? addresses.lockableV3Initializer;
  ...
  async createStaticAuction(
    ...
    const createParams =
      options?._createParams ?? (await this.encodeCreateStaticAuctionParams(params));
  ...
  async createDynamicAuction(
    ...
    const poolId = this.computePoolId({
      currency0:
        actualTokenAddress < params.sale.numeraire ? actualTokenAddress : params.sale.numeraire,
      currency1:
        actualTokenAddress < params.sale.numeraire ? params.sale.numeraire : actualTokenAddress,
      fee: params.pool.fee,
      tickSpacing: params.pool.tickSpacing,
    ...
    switch (config.type) {
      ...
      case "uniswapV4":
        ...
        return encodeAbiParameters(
          ...
          [config.fee, config.tickSpacing, streamableFees.lockDuration, beneficiaryData],
        );
  ...
  encodeCreateMulticurveParams(params: CreateMulticurveParams<C>): CreateParams {
    ...
    const baseInitializerParams = {
      ...
      beneficiaries: sortedBeneficiaries.map(
        (b: NonNullable<typeof params.pool.beneficiaries>[number]) => ({
          beneficiary: b.beneficiary,
          shares: b.shares,
        }),
  ...
  private normalizeMulticurveCurves(
    ...
    const sanitizedCurves = curves.map((curve) => {
      ...
      if (sanitized.tickLower <= 0 || sanitized.tickUpper <= 0) {
        console.warn(
          "Warning: Using negative or zero ticks in multicurve configuration. Please verify this is intentional before proceeding.",
        );
      }
   ...
   * @private
   */
  private computeCreate2Address(salt: Hash, initCodeHash: Hash, deployer: Address): Address {
  ...
  private async ensureMulticurveBundlerSupport(bundler: Address): Promise<void> {
    ...
    const supports = Boolean(
      bytecode &&
      MULTICURVE_BUNDLER_SELECTORS.every((selector) => bytecode.includes(selector.slice(2))),
    );
