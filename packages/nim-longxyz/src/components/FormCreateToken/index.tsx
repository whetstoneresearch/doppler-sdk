...
export default function ({ children, ...props }: ParentProps) {
  ...
  // async function deployToken(token: TokenFields): Promise<Doppler> {
  //   console.log('>> [page]: init creation')
  //   const deployedToken = await createToken({
    ...
    <form
      ...
      onSubmit={async e => {
        ...
        try {
          ...
          const token = await await createToken({
            tokenName: state.name.value,
            tokenSymbol: state.symbol.value,
          })
