...
describe('Doppler Pool Deployment', () => {
    ...
    it('should deploy a new Doppler pool', async () => {
        const { clients: { publicClient, walletClient }, addresses, } = testEnv;
        ...
        const result = await readWriteFactory.airlock.simulateWrite('create', config);
        ...
        const doppler = new ReadDoppler(config.poolKey.currency1, addresses.stateView, drift);
        const poolId = await doppler.getPoolId();
        expect(poolId).toBeDefined();
    });
});
//# sourceMappingURL=pool-deployer.test.js.map