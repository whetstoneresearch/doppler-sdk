...
describe('Multicurve (Ethereum Sepolia fork) smoke test', () => {
  if (!hasRpcUrl(CHAIN_IDS.ETH_SEPOLIA)) {
    it.skip(`requires ${getRpcEnvVar(CHAIN_IDS.ETH_SEPOLIA)} env var`)
    return
  }
  ...
  it('has non-zero Uniswap V4 trading endpoints configured for Ethereum Sepolia', () => {
  ...
  it('verifies whitelisted module states for configured Ethereum Sepolia modules', async () => {
  ...
  it('defaults multicurve governance to noOp on Ethereum Sepolia', () => {
    const params = sdk
      .buildMulticurveAuction()
      .tokenConfig({
        type: 'standard',
        name: 'SepoliaGovernanceDefault',
        symbol: 'SGD',
        tokenURI: 'ipfs://sepolia-governance-default',
      })
      .saleConfig({
  ...
  it('simulates create when required modules exist, otherwise fails fast with a clear config error', async () => {
    const params = sdk
      ...
      .tokenConfig({
        type: 'standard',
        name: 'SepoliaCreatePath',
        symbol: 'SCP',
        tokenURI: 'ipfs://sepolia-create-path',
      })
      .saleConfig({
        initialSupply: 1_000_000n * WAD,
        numTokensToSell: 1_000_000n * WAD,
