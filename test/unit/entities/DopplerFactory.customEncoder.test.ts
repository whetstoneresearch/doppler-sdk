...
} from '../../../src/entities/DopplerFactory';
import { CHAIN_IDS } from '../../../src/addresses';
import type { MigrationConfig, CreateStaticAuctionParams } from '../../../src/types';
import type { SupportedPublicClient } from '../../../src/types';
import { TICK_SPACINGS } from '../../../src/constants';
...
describe('DopplerFactory Custom Migration Encoder', () => {
  ...
  beforeEach(() => {
    ...
    customEncoder = vi.fn((_config: MigrationConfig) => {
  ...
  it('should use custom migration encoder when provided', async () => {
    ...
    const result =
      await factory.encodeCreateStaticAuctionParams(mockCreateParams);
  ...
  it('should fall back to default encoding when no custom encoder provided', async () => {
    ...
    const defaultFactory = new DopplerFactory(
      publicClient,
      undefined,
      CHAIN_IDS.BASE_SEPOLIA,
    );
    ...
    const result =
      await defaultFactory.encodeCreateStaticAuctionParams(mockCreateParams);
  ...
  it('returns empty migration payload for default V3 config', async () => {
    ...
    const result =
      await defaultFactory.encodeCreateStaticAuctionParams(paramsWithDefaultV2);
    expect(result.liquidityMigratorData).toBe('0x');
  });
  ...
  it('returns empty migration payload for default V4 config', async () => {
    ...
    const result =
      await defaultFactory.encodeCreateStaticAuctionParams(paramsWithDefaultV4);
    expect(result.liquidityMigratorData).toBe('0x');
  });
  ...
  it('encodes migration payload when V4 streamable fees configured', async () => {
    ...
    const paramsWithStreamableV4 = {
      ...
      migration: {
        ...
        streamableFees: {
          ...
          beneficiaries: [
            {
              beneficiary: mockCreateParams.userAddress,
              shares: parseEther('1'),
            },
          ],
        },
    ...
    const result = await defaultFactory.encodeCreateStaticAuctionParams(
      paramsWithStreamableV4,
    );
  ...
  it('should support fluent method chaining', async () => {
    const mockEncoder: MigrationEncoder = vi.fn(
      () => '0xtest' as `0x${string}`,
    );

    // Test that withCustomMigrationEncoder returns the factory instance for chaining
    const chainedFactory = new DopplerFactory(
      publicClient,
      undefined,
      CHAIN_IDS.BASE_SEPOLIA,
    ...
    const result =
      await chainedFactory.encodeCreateStaticAuctionParams(mockCreateParams);
