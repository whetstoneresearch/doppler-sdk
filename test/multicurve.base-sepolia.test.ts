...
describe("Multicurve (Base Sepolia fork) smoke test", () => {
  ...
  it("can simulate create() for multicurve with a non-zero fee", async () => {
    ...
    const zeroFeeBuilder = sdk
      ...
      .tokenConfig({
        type: "standard",
        name: "MultiCurveFeeZero",
        symbol: "MC0",
        tokenURI: "ipfs://fee-zero",
      })
      .saleConfig({
        initialSupply: 1_000_000n * WAD,
        numTokensToSell: 1_000_000n * WAD,
  ...
  it("matches doppler multicurve initializer non-zero fee parameters", async () => {
    ...
    const builder = sdk
      ...
      .tokenConfig({
        type: "standard",
        name: "MultiCurveFee3000",
        symbol: "MCF3",
        tokenURI: "ipfs://fee-3000",
      })
      .saleConfig({
        initialSupply: 1_000_000n * WAD,
        numTokensToSell: 1_000_000n * WAD,
  ...
  it("quotes multicurve bundle via the Bundler helpers", async () => {
    ...
    expect(
      initializerWhitelisted &&
        migratorWhitelisted &&
        tokenFactoryWhitelisted &&
        governanceFactoryWhitelisted,
    ).toBe(true);
    ...
    const builder = sdk
      ...
      .tokenConfig({
        type: "standard",
        name: "MultiCurveBundle",
        symbol: "MCB",
        tokenURI: "ipfs://bundle",
      })
      .saleConfig({
        initialSupply: 1_000_000n * WAD,
