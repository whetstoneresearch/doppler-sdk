...
describe("Static Auction with lockable beneficiaries (Base Sepolia)", () => {
  ...
  it("can encode static auction params with lockable beneficiaries", async () => {
    ...
    if (states.v3Initializer !== 3) {
      console.warn(
        "V3 initializer not whitelisted on Base Sepolia (state=%d), skipping test",
    ...
    expect(
      sortedBeneficiaries[0].beneficiary.toLowerCase() <
        sortedBeneficiaries[1].beneficiary.toLowerCase(),
    ).toBe(true);
    expect(
      sortedBeneficiaries[1].beneficiary.toLowerCase() <
        sortedBeneficiaries[2].beneficiary.toLowerCase(),
    ).toBe(true);
  ...
  it("automatically sorts beneficiaries by address", async () => {
    ...
    expect(
      beneficiaries[0].beneficiary.toLowerCase() < beneficiaries[1].beneficiary.toLowerCase(),
    ).toBe(true);
    expect(
      beneficiaries[1].beneficiary.toLowerCase() < beneficiaries[2].beneficiary.toLowerCase(),
    ).toBe(true);
